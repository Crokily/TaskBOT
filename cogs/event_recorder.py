...
class EventRecorder(commands.Cog):
    ...
    async def stop_recording(self, guild_id):
        ...
        try:
            ...
            # 上传录音文件
            await self.upload_recording(guild_id, temp_file, event)
    ...
    async def upload_recording(self, guild_id, file_path, event):
        """上传录音文件到Supabase"""
        try:
            ...
            supabase.table("Meetings Records").insert({
                "Meeting ID": f"event_{int(time.time())}",
            ...
            # 将会议记录保存为JSON文件
            json_file_path = os.path.join(os.path.dirname(file_path), f"{meeting_name}.json")
